...
import { insuranceVehicle } from "../../../utils/auth.service";
import { BASE_URL } from "../../../config";
export default function RegisterUploadInsurencePhotoScreen({ navigation }) {
  ...
  const next = () => {
    ...
    if (image == "") {
      setloading(false);
      console.log("RegisterUploadRegistrationPhotoScreen", copy);
    } else {
      console.log("calling");

      insuranceVehicle(image)
        .then((res) => {
          console.log("resresresresresres>>>", res?.data?.data);
          let newdriver = {
            ...copy_driver,
            vehicle_insurance: res?.data?.data,
          };
          if (res?.data?.data) {
            dispatch(setRegisterData({ ...copy, driver: newdriver }));
            navigation.navigate("RegisterUploadRegistrationPhotoScreen");
          }
        })
        .catch((e) => {
          console.log("errerrerrerrerereerer>>>", e);
        })
        .finally(() => {
          setloading(false);
        });
